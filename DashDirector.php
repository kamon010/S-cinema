...
// คำสั่ง SQL เพื่อดึงข้อมูล DirectorID และ NameDirector จากตาราง Director
$sql_directors = "SELECT DISTINCT d.DirectorID, d.NameDirector 
               FROM Director d";
$result_directors = $conn->query($sql_directors);

// ตรวจสอบว่ามีการเลือกผู้กำกับหรือไม่
if (isset($_POST['directors']) && !empty($_POST['directors'])) {
    $selected_director_id = $_POST['directors'];

    // คำสั่ง SQL เพื่อดึง NameDirector ของผู้กำกับที่เลือก
    $sql_director_info = "SELECT NameDirector 
                       FROM Director
                       WHERE DirectorID = '$selected_director_id' LIMIT 1";
    $result_director_info = $conn->query($sql_director_info);

    // ตรวจสอบว่าพบข้อมูลหรือไม่
    if ($result_director_info && $result_director_info->num_rows > 0) {
        $row_director_info = $result_director_info->fetch_assoc();
        $selected_director = $row_director_info['NameDirector'];

        // คำสั่ง SQL เพื่อดึงหนังที่ผู้กำกับนี้กำกับ และมีข้อมูลยอดขาย
        $sql_movies = "SELECT m.MovieID, m.NameMovie
                       FROM Movie m
                       WHERE m.DirectorID = '$selected_director_id' AND m.MovieID IN (SELECT DISTINCT MovieID FROM Bookings)";
...
<body>
    <nav>
        ...
        echo '<select name="directors" id="directors" class="nav-link" style="background-color: #424242; color: #FFFFFF; width: 150px;">';
        echo '<option value="" disabled selected>Director</option>'; // เพิ่มตัวเลือกแรก
        while ($row_directors = $result_directors->fetch_assoc()) {
            echo '<option value="' . $row_directors['DirectorID'] . '">' . $row_directors['NameDirector'] . '</option>';
        }
        echo '</select>';
    ...
    <?php if (isset($selected_director) && isset($movie_sales_data)) { ?>
        <div>
            <h4>Total Sales of Movies Directed by <?php echo $selected_director; ?></h4>
            <?php $count = 0; ?>
