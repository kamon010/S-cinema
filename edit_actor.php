...
if (!isset($_GET['id'])) {
    // ถ้าไม่ได้ระบุ ID ของนักแสดงให้ redirect กลับไปที่หน้า all_actors.php หรือหน้าที่ต้องการ
...
try {
    ...
    // ดึงข้อมูลของนักแสดงจากฐานข้อมูลโดยใช้ ActorID
    $sql = "SELECT * FROM Actors WHERE ActorsID = :actors_id"; 
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':actors_id', $actor_id);
    $stmt->execute();
    $actor = $stmt->fetch(PDO::FETCH_ASSOC); // เพิ่มบรรทัดนี้เพื่อดึงข้อมูลนักแสดง
    if (!$actor) {
        // หากไม่พบข้อมูลนักแสดงให้ redirect กลับไปที่หน้า all_actors.php หรือหน้าที่ต้องการ
        header("Location: all_actors.php");
        exit();
    }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark justify-content-center">
    ...
    <div class="container">
        ...
        <form action="update_actor.php" method="post">
            ...
            <input type="hidden" name="actors_id" value="<?php echo $actor['ActorsID']; ?>">
            <div class="form-group">
                <label for="name">Name:</label>
                <input type="text" class="form-control" id="name" name="name" value="<?php echo $actor['NameActor']; ?>">
            </div>
            <div class="form-group">
                <label for="image_url">Image URL:</label>
                <input type="text" class="form-control" id="image_url" name="image_url" value="<?php echo $actor['ImageURL']; ?>">
            </div>
